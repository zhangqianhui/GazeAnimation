...
class Dataset(object):
    ...
    def input2(self):
        ...
        traindata_x = tf.data.Dataset.from_tensor_slices((self.train_images_list, self.train_m_list, self.train_eye_pos))
        traindata_y = tf.data.Dataset.from_tensor_slices((self.train_images_list2, self.train_m_list2, self.train_eye_pos2))
        testdata_y = tf.data.Dataset.from_tensor_slices((self.test_images_list, self.test_m_list, self.test_eye_pos))

        def getDataset(dataset, shuffle=True, shuffle_buffer_size=2048):

            def _map_fn(path1, path2, label):
                img = tf.read_file(path1)
                img = tf.image.decode_png(img, 3)
                img = tf.image.resize_images(img, size=(self.height, self.width))
                img = tf.clip_by_value(img, 0, 255) / 127.5 - 1

                mask = tf.image.decode_png(tf.read_file(path2), 3)
                mask = tf.image.resize_images(mask, size=(self.height, self.width))
                mask = tf.clip_by_value(mask, 0, 255) / 255.0
            ...
            if shuffle:
                dataset = dataset.repeat(None).prefetch(2)
            else:
                dataset = dataset.repeat(1).prefetch(2)

            return dataset

        _traindata_x = getDataset(traindata_x, shuffle=True)
        _traindata_y = getDataset(traindata_y, shuffle=True)
        _testdata_y = getDataset(testdata_y, shuffle=False)

        return _traindata_x, _traindata_y, _testdata_y

    def input3(self):

        traindata_z = tf.data.Dataset.from_tensor_slices((self.train_images_list + self.train_images_list2
                                                          , self.train_m_list + self.train_m_list2
                                                          , self.train_eye_pos + self.train_eye_pos2))
        traindata_x = tf.data.Dataset.from_tensor_slices((self.train_images_list, self.train_m_list, self.train_eye_pos))
        #traindata_y = tf.data.Dataset.from_tensor_slices((self.train_images_list2, self.train_m_list2, self.train_eye_pos2))
        testdata_y = tf.data.Dataset.from_tensor_slices((self.test_images_list, self.test_m_list, self.test_eye_pos))
        ...
        _traindata_z = getDataset(traindata_z, shuffle=True)
        _traindata_x = getDataset(traindata_x, shuffle=True)
        #_traindata_y = getDataset(traindata_y, shuffle=True)
        _testdata_y = getDataset(testdata_y, shuffle=False)

        return _traindata_x, _traindata_z, _testdata_y
