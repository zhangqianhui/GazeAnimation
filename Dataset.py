...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        self.train_images_list, self.train_eye_pos, self.train_images_list2, self.train_eye_pos2, \
        self.test_images_list, self.test_eye_pos, self.test_images_list0, self.test_eye_pos0 = self.readfilenames()
    ...
    def read_images(self, input_queue):
        ...
        content = tf.compat.v1.read_file(input_queue)
        # Decode a JPEG-encoded image to a uint8 tensor
        image = tf.image.decode_jpeg(content, channels=self.channel)
        image = tf.cast(image, tf.float32)
        image = tf.compat.v1.image.resize_images(image, size=(self.height, self.width))
    ...
    def input(self):
        ...
        train_queue = tf.compat.v1.train.slice_input_producer([train_images, train_eye_pos], shuffle=True)
        ...
        train_queue2 = tf.compat.v1.train.slice_input_producer([train_images2, train_eye_pos2], shuffle=True)
        ...
        test_queue = tf.compat.v1.train.slice_input_producer([test_images, test_eye_pos], shuffle=False)
        test_eye_pos_queue = test_queue[1]
        test_images_queue = self.read_images(input_queue=test_queue[0])

        batch_image1, batch_eye_pos1, batch_image2, batch_eye_pos2 = tf.compat.v1.train.shuffle_batch([train_images_queue,
                                                                                                       train_eye_pos_queue,
                                                                                                       train_images_queue2,
                                                                                                       train_eye_pos_queue2],
                                                                                                       batch_size=self.batch_size,
                                                                                                       capacity=self.capacity,
                                                                                                       num_threads=self.num_threads,
                                                                                                       min_after_dequeue=500)

        batch_image3, batch_eye_pos3 = tf.compat.v1.train.batch([test_images_queue, test_eye_pos_queue],
                                                                batch_size=self.batch_size,
                                                                capacity=100,
                                                                num_threads=1)
